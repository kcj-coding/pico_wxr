import machine
from machine import Pin, I2C        #importing relevant modules & classes
#from time import sleep
import time
import utime
#import umail
import socket
import network
from breakout_bme280 import BreakoutBME280
from pimoroni_i2c import PimoroniI2C
from pimoroni import PICO_EXPLORER_I2C_PINS
from picographics import PicoGraphics, DISPLAY_PICO_EXPLORER
#import bme280                       #importing BME280 library
#import pico_wxr1
#import temp_internal1

i2c = PimoroniI2C(**PICO_EXPLORER_I2C_PINS)
bme = BreakoutBME280(i2c, address=0x76)

sensor_temp = machine.ADC(4)
conversion_factor = 3.3 / (65535)

rtc=machine.RTC()
#timestamp=rtc.datetime()

#timer = 0
#i2c=I2C(0,sda=Pin(17), scl=Pin(16), freq=400000)    #initializing the I2C method

wlan = network.WLAN(network.STA_IF)
wlan.active(True)
wlan.connect("SSID","Password")


def web_page():
    #bme = BreakoutBME280(i2c, address=0x76)#bme280.BME280(i2c=i2c)          #BME280 object created
    html = """<html><head><meta http-equiv="refresh" content="5"><meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="icon" href="data:,"><style>body { text-align: center; font-family: "Helvetica", Arial;}
  table { border-collapse: collapse; width:55%; margin-left:auto; margin-right:auto; }
  th { padding: 12px; background-color: #87034F; color: white; }
  tr { border: 2px solid #000556; padding: 12px; }
  tr:hover { background-color: #bcbcbc; }
  td { border: none; padding: 14px; }
  .sensor { color:DarkBlue; font-weight: bold; background-color: #ffffff; padding: 1px;  
  </style></head><body><h1>Kelvin's BME280 Pi Pico W Weather Station</h1>
  <table><tr><th>Parameters</th><th>Current Value</th><th>Initial Value</th><th>Change</th><th>Change(inc/dec)</th></tr>
  <tr><td>Temperature</td><td><span class="sensor">""" + str(round(temperature,3))+"C"+"""</td><td>""" + str(round(init_temperature,3)) +"C"+"""</td><td>"""+str(round(temperature-init_temperature,3))+"C"+ """</td><td>"""+incdec(round(temperature-init_temperature,3))+""+ """</span></td></tr></span></td></tr>
  <tr><td>Pressure</td><td><span class="sensor">""" + str(round(pressurehpa,2)) + 'hPa' +"""</td><td>""" + str(round(init_pressurehpa,2)) +"hPa"+"""</td><td>"""+str(round(pressurehpa-init_pressurehpa,2))+"hPa"+ """</td><td>"""+incdec(round(pressurehpa-init_pressurehpa,2))+""+ """</span></td></tr></span></td></tr>
  <tr><td>Humidity</td><td><span class="sensor">""" + str(round(humidity,2)) + '%' +"""</td><td>""" + str(round(init_humidity,2)) +"%"+"""</td><td>"""+str(round(humidity-init_humidity,2))+"%"+ """</td><td>"""+incdec(round(humidity-init_humidity,2))+""+ """</span></td></tr></span></td></tr>
  <tr><td>CPU Temp</td><td><span class="sensor">""" + str(round(temperature_internal,2))+"C" +"""</td><td>""" + str(round(init_temperature_internal,2)) +"C"+"""</td><td>"""+str(round(temperature_internal-init_temperature_internal,2))+"C"+ """</td><td>"""+incdec(round(temperature_internal-init_temperature_internal,2))+""+ """</span></td></tr></span></td></tr>
  <tr><td>Runtime</td><td><span class="string">""" + str(timestring)+ """</span></td></tr>

  <head><meta http-equiv="refresh" content="5"><meta name="viewport" content="width=device-width, initial-scale=1"><style>img{display: block; margin-left: auto; margin-right: auto;}</style></head><body><img src="data:image/jpeg;base64," alt="pico" style="max-width:100%;">
  </body></html>"""
    return html

def incdec(value):
    if value > 0:
        return "Increase"
    elif value == 0:
        return "No change"
    else:
        return "Decrease"

# Wait for connect or fail
wait = 10
while wait > 0:
    if wlan.status() < 0 or wlan.status() >= 3:
        break
    wait -= 1
    print('waiting for connection...')
    time.sleep(1)


# Handle connection error
if wlan.status() != 3:
    raise RuntimeError('wifi connection failed')
else:
    print('connected')
    ip=wlan.ifconfig()[0]
    print('IP: ', ip)

# Open socket
addr = socket.getaddrinfo('0.0.0.0', 80)[0][-1]
server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server.bind(('', 80))
server.listen(5)
print('listening on', addr)

init_reading = sensor_temp.read_u16() * conversion_factor
init_temperature_internal = 27 - (init_reading - 0.706)/0.001721

init_temperature, init_pressure, init_humidity = bme.read()
# pressure comes in pascals which is a reight long number, lets convert it to the more manageable hPa
init_pressurehpa = init_pressure / 100

start_time = time.time()

while True:
    try:
        #timer = timer+5
        #timestamp=rtc.datetime()
        #timestring="%02d:%02d:%02d"%(timestamp[4:7])
        timestring = time.time() - start_time
        (minutes, seconds) = divmod(timestring, 60)  
        (hours, minutes) = divmod(minutes, 60)  
        (days,hours) = divmod(hours, 24)
        timestring = "Day: "+str(days)+"| hours: "+f"{hours:02d}"+"| minutes: "+f"{minutes:02d}"+"| seconds: "+f"{seconds:02d}"+"|"
        #if timestring >= "23:59:59":
         #   Day = Day+1
        #timestring="Day "+str(Day)+" "+timestring
        reading = sensor_temp.read_u16() * conversion_factor 
        temperature_internal = 27 - (reading - 0.706)/0.001721

        temperature, pressure, humidity = bme.read()
        # pressure comes in pascals which is a reight long number, lets convert it to the more manageable hPa
        pressurehpa = pressure / 100
        #timestring="%04d-%02d-%02d %02d:%02d:%02d"%(timestamp[0:3] +
        #                                        timestamp[4:7])
        conn, addr = server.accept()
        conn.settimeout(3.0)
        print('client connected from', addr)
        request = conn.recv(1024)
        conn.settimeout(None)
        # HTTP-Request receive
        print('Request:', request)              
        # HTTP-Response send
        response = web_page()
        conn.send('HTTP/1.0 200 OK\r\nContent-type: text/html\r\n\r\n')
        conn.sendall(response)
        conn.close()
    except OSError as e:
        conn.close()
        print('connection closed')

